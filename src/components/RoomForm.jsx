...
const RoomForm = () => {
  ...
  const addChild = () => {
    if (isPositiveInteger(childAge) && isPositiveInteger(childNumber)) {
      ...
      const totalChildren =
        children.reduce((acc, child) => acc + child.number, 0) + number;

      if (totalChildren > 10) {
        const isConfirmed = window.confirm(
          "You have added more than 10 children. Are you sure this is correct?"
        );
        if (!isConfirmed) {
          return;
        }
      ...
      <div className="flex flex-col md:flex-row md:space-x-4 p-4 mr-1">
        <div className="flex-1 bg-gray-50-custom text-black p-4 rounded-md mb-4 md:mb-0">
          ...
          <div className="flex flex-col mt-4">
            ...
            <div className="flex flex-col w-full lg:flex-row items-center md:space-x-4 gap-5">
              <input
                type="text"
                id="childAge"
                value={childAge}
